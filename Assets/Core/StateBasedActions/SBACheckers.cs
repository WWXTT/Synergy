...
namespace CardCore
{
    ...
    /// 传说规则检查器
    /// </summary>
    public class LegendaryRuleChecker : ISBAChecker
    {
        public void Check(StateBasedActions sba, GameCore gameCore)
        {
            foreach (var player in new[] { gameCore.Player1, gameCore.Player2 })
            {
                var battlefield = gameCore.ZoneManager.GetCards(player, Zone.Battlefield);
                var legends = battlefield.Where(c => c is IHasCardType ct && ct.CardType == CardType.传奇);

                var groupedByName = legends.GroupBy(c => c.ID);
                foreach (var group in groupedByName)
                {
                    if (group.Count() > 1)
                    {
                        // 销毁后入场的（时间戳较新的）
                        var toDestroy = group.OrderByDescending(c => c.TimestampInfo.Sequence).First();
                        sba.AddAction(new SBAActionRecord
                        {
                            Type = SBAActionType.ControlChange,
                            AffectedEntity = toDestroy
                        });
                    }
                }
            }
